...
const CoffeeMenu: Component = () => {
  ...
  return (
    <section class="py-8 min-h-screen" aria-label="Coffee menu">
      ...
      <h2 class="text-3xl mb-8 text-center text-gray-800">Our Coffee Menu</h2>
      <div class="flex justify-center gap-4 mb-8">
        <For each={socialLinks}>
          {(link) => (
            ...
            >
              <span class="absolute inset-0 rounded-full border-2 border-white/20 group-hover:border-white/40 transition-all"></span>
              <span class="absolute inset-0 rounded-full bg-gradient-to-br from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></span>
              {getIcon(link.icon)}
            </a>
          )}
        </For>
      ...
      <div class="grid grid-cols-1 gap-4 mt-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <For each={menu.items}>
          {(item) => (
